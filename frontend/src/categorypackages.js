...
const CategoryPackages = () => {
  ...
  useEffect(() => {
    axios.get(`https://www.thepilgrimbeez.com/backend/packages/${encodeURIComponent(category)}`)
       .then(response => {
                                ...
                                <Link to={`/package/id/${pkg.id}`}>
                                    <div className="card">
                                        <span className="over_hover">
                                            <img src={`https://www.thepilgrimbeez.com/backend/uploads/${pkg.imageUrl}`} alt={pkg.packageName || 'Package Image'} className="card-img" /></span> {/* Corrected className */}
